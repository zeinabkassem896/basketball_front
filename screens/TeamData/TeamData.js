...
import {
  ...
  import { MyTeamStyle } from "../MyTeam/MyTeamStyle";
...
const MyTeam = ({ route, navigation  }, props) => {
    ...
    const [team_id, setTeamId] = useState(route.params.team_id);
  ...
  const getPlayersTeamData = async()=>{
    try{
      await axios(`${API_LINK}/api/players-team/${team_id}`).then((response) => {
  ...
  const getTeamScoresData = async()=>{
    try{
      await axios(`${API_LINK}/api/team-scores/${team_id}`).then((response) => {
          ...
          <View style={MyTeamStyle.NewsDetailBackground}>
              <ImageBackground source={require('../../assets/My_Team_BG.png')} style={MyTeamStyle.ArticleImgBackground}>
                  <View style={MyTeamStyle.titleDiv}>
                      ...
                      <Text style={MyTeamStyle.title}>{route.params.team_name} Club</Text>
